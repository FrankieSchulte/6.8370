...
Image cornerResponse(const Image &im, float k, float sigmaG,
  ...
  Image newImage = Image(im.width(), im.height(), im.channels());
  newImage = computeTensor(im, sigmaG, factorSigma);
  Image corner = Image(im.width(), im.height(), 1);

  for (int x = 0; x < im.width(); x++) {
    for (int y = 0; y < im.height(); y++) {
      Matrix M = Matrix::Zero(2, 2);

      M(0, 0) = newImage(x, y, 0);
      M(0, 1) = newImage(x, y, 1);
      M(1, 0) = newImage(x, y, 1);
      M(1, 1) = newImage(x, y, 2);
...
vector<Point> HarrisCorners(const Image &im, float k, float sigmaG,
  ...
  Image corner = cornerResponse(im, k, sigmaG, factorSigma);
  Image maximum = maximum_filter(corner, maxiDiam);

  for (int x = 0; x < im.width(); x++) {
    for (int y = 0; y < im.height(); y++) {
      if (corner(x, y) == maximum(x, y) && corner(x, y) > 0) {
...
Image descriptor(const Image &blurredIm, const Point &p,
  ...
  // 'radiusDescriptor'.
  float diameter = (2 * radiusDescriptor) + 1;
  Image result = Image(diameter, diameter, 1);
  for (int x = p.x; x < p.x + diameter; x++) {
    for (int y = p.y; y < p.y + diameter; y++) {
      result(x - p.x, y - p.y, 0) = blurredIm(x, y);
    }
  }

  float resultAverage = result.mean();
  float resultVar = result.var();
  for (int x = 0; x < result.width(); x++) {
    for (int y = 0; y < result.height(); y++) {
      result(x, y, 0) = (result(x, y, 0) - resultAverage) / sqrt(resultVar);
    }
  }
  return result;
}
...
vector<Feature> computeFeatures(const Image &im, const vector<Point> &cornersL,
  ...
  vector<Feature> result = vector<Feature>();
  Image lumi = lumiChromi(im)[0];
  Image gaussBlur = gaussianBlur_separable(lumi, sigmaBlurDescriptor);

  for (int i = 0; i < cornersL.size(); i++) {
    cout << "hi" << endl;
    Point p = cornersL[i];
    Image description = descriptor(gaussBlur, p, radiusDescriptor);
    result.push_back(Feature(p, description));
  }
  return result;
}
