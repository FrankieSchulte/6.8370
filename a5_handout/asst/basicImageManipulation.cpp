...
Image scaleNN(const Image &im, float factor) {
  ...
  Image newImage = Image(floor(im.width() * factor), floor(im.height() * factor), im.channels());
  for (int x = 0; x < floor(im.width() * factor); x++) {
    for (int y = 0; y < floor(im.height() * factor); y++) {
      for (int z = 0; z < im.channels(); z++) {
        newImage(x, y, z) = im(floor(x / factor), floor(y / factor), z);
      }
    }
  }
  return newImage;
...
float interpolateLin(const Image &im, float x, float y, int z, bool clamp) {
  ...
  float lowerLeft = im.smartAccessor(floor(x), floor(y), z, clamp) * (1 - abs(x - floor(x))); 
  float lowerRight = im.smartAccessor(ceil(x), floor(y), z, clamp) * abs(x - floor(x));
  float upperLeft = im.smartAccessor(floor(x), ceil(y), z, clamp) * (1 - abs(x - floor(x))); 
  float upperRight = im.smartAccessor(ceil(x), ceil(y), z, clamp) * abs(x - floor(x));

  float lower = ((lowerLeft + lowerRight) * (1 - abs(y - floor(y))));
  float upper = ((upperLeft + upperRight) * abs(y - floor(y)));
...
Image scaleLin(const Image &im, float factor) {
  ...
  for (int x = 0; x < floor(im.width() * factor); x++) {
    for (int y = 0; y < floor(im.height() * factor); y++) {
      for (int z = 0; z < im.channels(); z++) {
        newImage(x, y, z) = interpolateLin(im, floor(x / factor), floor(y / factor), z, true);
      }
    }
  }
  return newImage;
